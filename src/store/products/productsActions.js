...
export const deleteProduct = (id) => async (dispatch) => {
  ...
  commentList.forEach(element => {
    axios.get(baseURL + `/oldCarsComments.json?orderBy="id"&equalTo="${element}"&print=pretty`).then(({data}) => {
      return  axios.delete(baseURL + `/oldCarsComments/${Object.keys(data)[0]}.json`)
     }).then((response)=> axios.delete(baseURL + `/oldCars/${objectKey}.json`)).then(response => dispatch(deleteItem(id))).catch(err =>  alert('Some erros'));
  });
